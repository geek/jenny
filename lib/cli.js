...
exports.run = function (options, callback) {
    ...
    serial.start(function (err) {
        ...
        serial.on('register', function (data) {
            ...
            commander.register(data, function (err, res, result) {

                if (err) {
                    internals.handleError(err);
                }

                serial.writeId(result.id, internals.handleError);
            });
        });

        serial.on('sensor', function (data) {

            commander.report(data, internals.handleError);
        });

        serial.on('value', function (data) {
